...
class batchRenameFourDigitsBySizeDescendingTakeNameFromParentFolder(Command):
	def execute(self):
		current = self.fm.thisfile
		if current.is_directory:
			return self.fm.notify("Can only run this command on a file", bad = True)

		head, parentDirName = split(current.dirname)
		if(len(parentDirName) == 0):
			return self.fm.notify("parentDirName is empty", bad = True)

		parentDirName = "".join(parentDirName.split())
		logger.info("parentDirName: " + parentDirName)

		selection = self.fm.thistab.get_selection()
		selection.sort(key = attrgetter("size"), reverse = True)
		error = renameSequential(selection, parentDirName, 4)
		notify(self.fm, error, f"Renamed {len(selection)} files")
